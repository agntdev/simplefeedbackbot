...
async function sendBroadcast(ctx: Ctx): Promise<void> {
  ...
  const recipients = people.filter((user) => recipientIds.includes(user.telegram_id));
...
composer.callbackQuery(/^broadcast:ack:(\d+)$/, async (ctx) => { await ctx.answerCallbackQuery(); const result = await acknowledgeBroadcast(ctx, Number(ctx.match[1])); const ru = (await language(ctx)) === "ru"; await ctx.reply(result === "unavailable" ? (ru ? "Эта рассылка недоступна." : "That broadcast isn't available.") : (ru ? "Отмечено" : "Marked")); });
