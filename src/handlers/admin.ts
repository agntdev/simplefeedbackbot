...
async function adminKeyboard(ctx: Ctx) {
  ...
  return inlineKeyboard([[inlineButton(ru ? "Пользователи" : "Users", "admin:users:0"), inlineButton(ru ? "Статистика" : "Statistics", "admin:stats")], [inlineButton(ru ? "Экспорт CSV" : "Export CSV", "admin:export"), inlineButton(ru ? "Рассылка" : "Broadcast", "admin:broadcast")], [inlineButton(ru ? "Журнал действий" : "Audit log", "admin:audits")], [inlineButton(await tr(ctx, "backMenu"), "menu:main")]]);
}
...
async function sendBroadcast(ctx: Ctx): Promise<void> {
  ...
  await ctx.editMessageText(`Broadcast sent to ${delivered} user${delivered === 1 ? "" : "s"}.`, { reply_markup: await adminKeyboard(ctx) });
}
